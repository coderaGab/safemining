...
<div className="container-max relative z-10 text-center">
    ...
    <motion.h2 
      ...
      className="text-2xl md:text-4xl font-serif text-white mb-6"
    >
    ...
    <motion.a 
      ...
      whileTap={{ scale: 0.95 }}
      href="#contact" 
      className="bg-brand-accent text-brand-bg px-8 py-4 rounded font-bold hover:brightness-110 transition"
    >
...
<motion.div
      ...
      >
        <motion.div 
          ...
          className="bg-white p-8 rounded-2xl border border-brand-accent/10 shadow-sm hover:shadow-md transition-shadow"
        >
        ...
        <div className="space-y-4">
          {tabs[activeTab].items.map((item, i) => (
            ...
            >
              ...
              <p className="text-lg text-brand-text font-medium text-white">{item}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
...
<motion.div 
          whileHover={{ scale: 1.02 }}
          className="bg-white p-8 rounded-2xl border border-brand-accent/10 relative overflow-hidden shadow-sm"
        >
      ...
      <div className="space-y-4">
        {differentials.map((item, i) => (
          <motion.div 
            ...
            className="bg-white p-6 rounded-2xl border border-brand-accent/5 flex items-start space-x-6 hover:border-brand-accent/30 transition-all group shadow-sm"
          >
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-8 relative">
        {[
          ...
          <React.Fragment key={i}>
            ...
            >
              <div className="w-14 h-14 bg-brand-accent text-brand-bg rounded-full flex items-center justify-center shadow-lg shadow-brand-accent/30 transition-transform">
                {step.icon}
              </div>
...
<div>
<h2 className="text-4xl md:text-5xl font-serif font-bold text-white mb-8">O Risco <span className="text-brand-accent italic">Regulatório</span></h2>
...
<h4 className="font-bold text-white mb-2">{risk.title}</h4>
...
<div className="space-y-4">
{[
{ label: "Jan e Jul", task: "Emissão das guias da TAH" },
{ label: "Anual", task: "DIPEM - Declaração de Investimentos" },
{ label: "Anual", task: "RAL - Relatório Anual de Lavra" },
{ label: "Alvará", task: "Prorrogação do Alvará de Pesquisa" }
].map((item, i) => (
<div key={i} className="bg-white/5 p-4 rounded-xl border border-white/5 flex items-center justify-between">
<div className="flex items-center space-x-4">
<div className="px-3 py-1 bg-brand-accent text-brand-bg text-[10px] font-bold rounded uppercase">{item.label}</div>
...
<h2 className="text-4xl md:text-5xl font-serif font-bold text-brand-bg mb-6 text-brand-bg">
...
<div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
      {team.map((member, i) => (
        <motion.div 
          ...
          className="bg-white rounded-2xl p-10 shadow-sm border border-brand-accent/5 flex flex-col"
        >
...
<p className="text-brand-text-muted max-w-2xl mx-auto text-lg leading-relaxed">Como começar – 3 passos simples em menos de 30 dias.</p>
</div>
code
Code
<div className="grid md:grid-cols-3 gap-8">
      {steps.map((step, i) => (
        <motion.div 
          ...
          className="relative bg-brand-card p-10 rounded-3xl border border-white/10 text-center text-white"
        >
...
<div className="space-y-6">
          {[
            { icon: <Globe />, label: "Website", val: "www.safemining.com" },
            { icon: <Mail />, label: "E-mail", val: "contato@safemining.com" },
            { icon: <MapPin />, label: "Localização", val: "Atendimento em todo o Brasil" }
          ].map((item, i) => (
            <motion.div key={i} whileHover={{ x: 5 }} className="flex items-center space-x-5">
              <div className="w-12 h-12 bg-white/5 rounded-xl flex items-center justify-center text-brand-accent border border-white/5">
                {item.icon}
              </div>
              <div>
                <p className="text-xs font-bold text-white/50 uppercase tracking-widest mb-0.5">{item.label}</p>
                <p className={item.label === "Localização" ? "text-brand-text-muted font-medium" : "text-brand-accent font-medium"}>{item.val}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {[
            ...
            { label: "Empresa", name: "empresa", type: "text", placeholder: "Nome da empresa", required: false },
            { label: "E-mail", name: "email", type: "email", placeholder: "seu@email.com", required: true },
            { label: "Quantidade de processos", name: "processos", type: "text", placeholder: "Ex: 50 processos", required: false }
          ].map((field, i) => (
            <div key={i} className="space-y-2">
              <label className="text-xs font-bold text-white/70 uppercase tracking-widest">{field.label}</label>
              <input 
                type={field.type} 
                name={field.name}
          ...
          <motion.button 
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="w-full bg-brand-accent text-brand-bg py-5 rounded-xl text-sm font-bold uppercase tracking-widest hover:brightness-110 transition-all shadow-xl shadow-brand-accent/20"
          >
            Enviar Mensagem
          </motion.button>
        </form>
...
<p className="text-brand-text-muted max-w-sm leading-relaxed">
